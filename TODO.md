...
7.  **`q` (Stage 2 false positives) degrades hard as `K` rises -- now split into a closed trade and an open defect.** Found 29 July 2026 as a side effect of the M-ladder run (`PLAN.md` 13.7). Coverage falls from 0.945 at `M2` (K = 3) to **0.614 at `K30`**, and 0.742 at `M20`. Tested directly on 21 August 2026 (`PLAN.md` 21): four paired cells varying true `q`'s distance from its `Beta(1, 20)` prior mean (0.0476) crossed with K.
    ...
    **Still open, and now pointing at item 3's neighbourhood: K-linked overconfidence even when the prior is well placed.** The discriminating prediction was that truths hugging the prior mean would stay near nominal as K rises. They did not: 0.948 -> 0.605, matching the original ladder result almost exactly, with bias flat and tiny (-0.0001 to -0.004) while width contracts four-fold. Flat bias plus shrinking intervals under more detection-stage data is exactly the signature item 3 tracks for `beta_theta`, and both blocks are updated through Polya-Gamma machinery. Read this item together with item 3; Alex's reading that `sample_pq_cpp()` is correctly implemented is not contradicted -- the question is interval calibration under sharpening, shared with the sampler investigation below.

    ALEX OR CLAUDE: FOLD THE OPEN HALF INTO ITEM 3'S SAMPLER INVESTIGATION
