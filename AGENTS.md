...
Of the should-fix items: 7 and 15 are fixed; 11 is closed by deliberate decision (keep `LICENSE`, accept the cosmetic NOTE); 9 is pinpointed and re-verified but not fixed; 8, 10, 12, 13, 14 are open; and 17-20 are newly added and open. Item 19 **corrects** the earlier characterisation of the `tidyr` NOTE (it is a *missing-import*, not an *unused-import*, and dropping `tidyr` from `Imports` would be the wrong fix). The `stats`-import portion of the "no visible binding" NOTE is fixed (`ef77c4c`); the NSE-column portion is deliberately left unfixed by the user's decision; item 20 catalogues a further, previously untracked source of that NOTE (dead functions referencing undefined globals).

Recommended next steps, in order:

1. **Add the scratch `.rda` files to `.Rbuildignore`** -- one line, halves the item 4 blocker immediately.
2. **Fix `thinOutput()`** (TODO.Rmd B.2), then execute the item 4 thin+prune plan on the remaining 62 MB.
3. **Decide the OpenMP question (item 16)**: cap threads and make the RNG thread-safe, *or* drop OpenMP from `src/Makevars` and parallelise over chains in R instead (TODO.Rmd D.1). The second option closes item 16 and TODO.Rmd A.6 together and is the lower-risk path.
4. **Fix TODO.Rmd A.3** (`sample_pq_cpp()` counting), which clears 3 of item 9's 7 warnings as a side effect, then **delete the dead `buildGrid()`/`isPointInBand*` code**, which clears the other 4.
5. Add `URL`/`BugReports` (item 10) and the missing imports (item 19) -- both trivial.
6. **Rerun `devtools::check()` and `R CMD check --as-cran`** for a fresh baseline (item 13) before working through items 8, 12, 14, 17, 18, 20.
