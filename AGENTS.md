...
- **`sample_ls()` evaluates the wrong density, so the GP length-scale is never recovered** (the `sample_ls()` item in TODO.md group B). Found 27 July while writing the tier-1 tests. `idx_ls` walks to the top of `l_s_grid` and stays there for every true `l_s` tried, across seeds. Biases every `useSpatField = TRUE` fit. **Note the diagnosis changed**: this was originally filed as "`sigma_s` is never sampled", with a proposed fix of sampling it. That diagnosis was wrong and the fix is disproven -- the defect is in the density being scored, which is a modelling change, not a parameter addition. See TODO.md A.1 "What was ruled out".
- **`reparamFactorModel()` breaks the identity that residual covariance = `t(L) %*% L`** (the `reparamFactorModel()` item in TODO.md group B), inflating reported species correlations. **Disputed by Alex** -- see "Open disagreement" under Current work status.
- **Choose the informative Stage 2 priors.** Not a bug: an open modelling decision. Tracked as a group B item in TODO until Alex removed it in `093f2bb` without recording a chosen value; flagged there for him to confirm whether the decision is made. See the correction below.
...
One group B item is still only **partially** fixed: `thinOutput()` runs, but still thins 2-D posterior-mean matrices by row (dropping *sites*) and still drops the scalar `WAIC` (TODO.md *Fixed bugs* 16, which is logged as only partially fixed). The `computeSpeciesDetected()` residual was fully closed on 27 July (TODO.md Fixed bugs 23), as was the `sample_ls()` non-spatial crash regression (Fixed bugs 22).
...
**No em-dashes anywhere**, including chat and commit messages, not just these docs. They were also the single largest driver of the old wrap churn: pandoc writes an em-dash as three hyphens, two characters wider than the single character it replaces, so every wrap point after one shifted. With wrapping off that is harmless mechanically, but the rule stands regardless. Use `--`. This paragraph previously contained a literal em-dash as an illustration, which made it the only violation in the three docs; it is spelled out in words now.
...
### `sample_ls()`: four candidate causes ruled out

The entry originally blamed `sigma_s` being hard-coded to 1 and proposed sampling it. **That diagnosis was wrong and the fix is disproven.** Ruled out, in order:

- **Amplitude is not missing from the model.** `sigma_bs` *is* sampled (`R/jsdmfun.R:1211`) and *is* the SoR amplitude, since `Bs` carries prior variance `sigma_bs^2` and the kernel is built at unit amplitude. A separate `sigma_s` would be redundant and would create an identifiability problem.
- **Passing the correct amplitude does not help.** Profiling the grid at the fitted field's actual amplitude (`sd(SE) = 0.85`) instead of 1 leaves the likelihood monotone. The one-line fix the entry used to propose would not have worked.
- **`logDetKuu` is correct.** The factor is `* 2`, not `- 2`: `sum(log(rcppeigen_get_diag(K_xx))) * 2` matches the true log-determinant at all ten grid points, as it should for a Cholesky factor. `rcppeigen_get_diag()` returns the Cholesky diagonal despite the name.
- **Not an artefact of weak data.** Identical behaviour at `ds = 2` (real spatial signal) as at `ds = 0`.
...
The tier-1 test asserts that `sample_ls()` is *reached* rather than that `idx_ls` varies, deliberately, so it does not encode the defect. Revisit that test when this is fixed.

### `reparamFactorModel()`: the disagreement, in full

**Alex's position:** "THIS IS NOT AN ISSUE FOR LOGISTIC MODELS (as the covariance matrix is not recoverable anyway, only the correlation one)."

**The premise is right but does not resolve it.** In a logistic latent-variable model the scale is indeed unidentified and only the correlation is meaningful. But the measured numbers are already correlations, `cov2cor()` output, and they still move by up to 0.612. `cov2cor()` does not absorb the change because `diag(1/diag(R))` rescales **per factor** while `cov2cor()` normalises **per species**: a per-factor rescaling is not a global scale change, so it survives the normalisation.

**If it is nonetheless a non-issue, the simulation study is measuring the wrong thing and that needs identifying.** The study compares `cov2cor(crossprod(L_true))` from the simulator's own `L` against the same function of the stored `L`. Either that comparison is invalid, or the correlations really are inflated. Worth settling before any `resid_cor` result is quoted.
...
- **WARNING: package cannot be installed cleanly.** **The package's own C++ is now warning-free** as of 30 July: the bitwise-`&` sites and the unused `TRUNC`/`TRUNC_RECIP` constants are gone (the C++ warning-clearing change, commit `bc779d3`). What remains is **not ours**: the sole significant warning comes from R's own header, `R_ext/Boolean.h:62`, where a `#pragma` names a warning group this clang version does not recognise. That is an R-build and toolchain artifact, environment-specific, and not fixable from this package. **Do not chase it, and do not re-file it.**
- ~~**WARNING: `plotSpeciesResponseCurve.Rd` documents arguments the function does not have.**~~ **Gone as of 30 July:** Alex deleted the function and its `.Rd` in `093f2bb`, together with the group C item that tracked it. No dangling references remain in `R/`, `man/`, `NAMESPACE` or the vignettes. There is no *Fixed bugs* entry for the removal.
...
- **This session, 1 August 2026: doc maintenance, and it found that `PLAN.md` §12 had been deleted.** The whole "Results of the full run" section -- the headline R = 100 coverage table for all ten cells, the pre-fix table at 12.6, and the `B0`/`theta0` findings -- was removed by `a8f9725` on 30 July, a commit of mine whose message described only the TODO rewrite and said nothing about deleting it. Nine references to §12 survived across the three docs, including `PLAN.md`'s own "reading order if you are new to this: §12 for the current results", which would have sent Alex to a section that was not there.
  ...
  **The same commit also dropped two open group B items**, found by the same sweep: `theta0` overcovering at 0.978-0.985, and `q` degrading to 0.614 at `K30`. Neither was fixed, neither got a *Fixed bugs* entry, and both were still referenced as live from `PLAN.md` and from this file, which is what exposed them. Both restored, condensed to the current to-do format rather than the old investigation-log form. A third item, "choose the informative Stage 2 priors", was removed separately by Alex in `093f2bb` without a recorded decision; that one is his own note, so it is flagged in TODO for him to confirm rather than restored.

  **Then the cross-references were audited, and most of them were wrong.** Detail under "Cross-referencing TODO items" above. Every `group X item N` reference in the three docs and in two `R/` source comments has been converted to a subject reference. The rule was already written down for *fixed* items; it had no answer for open ones, which is the gap that let this accumulate.

  **The review queue in section A is empty, and that is the largest open question here.** It was built on 30 July as the complete list of Claude's code changes for Alex to check, nine items. Alex removed **all nine** across `093f2bb` and `6722e22` on the same day, with no `ALEX RESPONSE` annotations, no review notes, and no *Fixed bugs* entries. The single item there now is the `RcppParallel` linking fix, added afterwards by Doug in `c22a7d5`. Seven of the nine changes are recorded in *Fixed bugs* (28, 31-36) so the changes themselves are documented, but **whether Alex reviewed them or simply cleared the section is not recorded anywhere**, and that is a question for Doug rather than something to assume either way.

  **Two of the nine had no record at all** until 1 August: the ten-function deprecation, cited only by section-A position which has since been reused for a different item, and `listPriors$b_betatheta_slope_var`. The latter is the worse of the two: a new user-facing argument, live in `R/runOccJSDM.R:783-785`, with nothing in `TODO.md` tracking it and **no mention in the `@param listPriors` roxygen block**, which listed only the three original priors. It was reachable but undiscoverable for two days. Now *Fixed bugs* 37 and 38, and documented.

  **Three lessons.** First, the deletions happened in a commit that rewrote three documents at once; a large doc commit needs its section and item inventory checked before and after, exactly the way a code commit needs its tests. `grep -c` on the item lists would have caught all three. Second, the commit message is not a record of what a commit did unless someone checks -- the same failure recorded under the July 27 entry below, where three of Alex's claimed fixes did not match the code, and it applies to my own commits too. Third, an item deleted without a *Fixed bugs* entry is indistinguishable from an item that was fixed, which is exactly the failure mode the "write the Fixed bugs entry first" rule exists to prevent; it was written for Alex's deletions and I then broke it myself.

- **This session, 31 July 2026: ran Alex's `ncov_theta = 0` discriminator, at R = 50 and again at R = 200.** Results in `PLAN.md` §15.4-§15.6. Two open items moved.

  **`beta_theta` (the slope-overconfidence item in group B) is narrowed to the slopes, which is the useful result.** With `ncov_theta = 0` the block reduces to its intercept row, and that row covers at **0.968** against nominal 0.95. So the overconfidence tracked for a week is not a property of `beta_theta` as a block; it is specific to slope estimation. That is a much smaller target for Alex than "beta_theta undercovers", and it is the fourth mechanism-level question the study has answered that code reading did not.

  **`B0` (the bias-doubled item in group B) is mostly but not entirely downstream.** Removing the slopes removes 70% of the bias. At R = 50 the residual was 1.5 SE and §15.4 called it suggestive; the R = 200 confirmation puts it at **3.3 SE**, so it is real and the item stays open rather than closing as a consequence of item 4. `binary` at 1.0 SE from zero is the comparison for what "no bias" looks like here. Alex predicted the item would close entirely; it does not, and the confirmation run is what settles that rather than an argument either way.

- **This session, 30 July 2026 (later): cleaned up the three docs and finished several group C and D items.** `TODO.md` is down from 738 to about 520 lines and is now a to-do list rather than an investigation log. Section A is the complete queue of Claude's code changes for Alex to review; the evidence that justified each group B conclusion moved here, under "Evidence behind the group B items". Every `group A item N` cross-reference in `PLAN.md` was stale after the section renumbering and now points at group B or at a *Fixed bugs* number.

  **Completed this session:** `predictNewSites()` defaults plus three downstream defects it exposed; a stale-fit guard on the two GAM effect functions; the three missing imports, which turned out to be a latent runtime failure rather than a NOTE; the C++ `&`-to-`&&` change and two unused constants; and ten dead functions moved to `deprecated/`. The undefined-globals NOTE fell from 84 symbols to 65, and the package now contributes zero compiler warnings.

  **Two diagnoses worth remembering, because both overturned an assumption.** The vignette failure was never a plotting bug: it was the shipped `sampleresults.rda` predating `infos$X0_psi`, and it presented as a bug in whichever function touched it first, migrating to the next one each time one was "fixed". Two functions were investigated and one needlessly suspected before the data was. Separately, the `theta0` experiment disproved its own hypothesis while incidentally identifying the cause of the `B0` bias, which had been filed as "cause not yet identified" for two days.

  **Process note.** A `python3` heredoc that fails an `assert` still lets a chained `&& git commit` run, which produced one commit whose message described work that was not in it. It was unpushed and was split into two honest commits. Do not chain a commit after an edit script that can fail; check `git status` in between.
...
- **This session, later still on July 29 2026: ran the M ladder Doug commissioned for the `beta_theta`, `theta0` and `B0` items in group B.** 250 fits, R = 50, 189 min, 0 failures. Validity check passed: the `M2` arm agrees with the existing base cell within noise. Results in `PLAN.md` 13.7.
  ...
  **One finding the ladder was not built to answer**: `q` degrades hard with `K` (0.945 -\> 0.614 at `K30`), worse than any M arm. Filed as its own group B item, with a hypothesis that it is the same prior-driven cost-of-identifiability pattern as `beta_theta`, sharpened by more PCR replicates.
...
- **Provenance of the group A items (moved here from TODO.md, 29 July).** *Note on naming: "group A" in this and the following historical entries means the **inference-affecting bug** group, which became **group B** on 30 July when section A was repurposed as the queue of Claude's changes for review. Numbered references have been rewritten; the prose ones below have not.* TODO is now to-dos only; this is the history behind them. Of the six group A items that existed after the coverage study, **three were found by the study rather than by reading code** -- the collection-covariate prior mean, `reparamFactorModel()` breaking the residual-correlation identity, and the Stage 2 detection prior. A static audit had already been over the same files and walked past all three. That is the case for the simulation suite in one line, and the reason the *MEE paper* item treats re-running it after fixes as evidence rather than repetition.
...
- **This session (July 28 2026): ran the full R = 100 coverage study; all 10 cells, 1000 fits, 0 failures, \~497 min.** Two parts: `base` alone on 27 July (100 fits, 22.9 min) to settle the open empirical question first, then the other 9 cells on 28 July (900 fits, 474 min). Results in `dev/simstudy/results/` and tabulated in `PLAN.md` §12. **Three of the four defects it found are now traced to specific lines of code** (the `reparamFactorModel()`, `beta_theta` and `theta0` items in group B); the fourth (`low_information` degraded across the board) is those three compounding when data are thin.
...
- **Earlier (July 27 2026): built the test suite (stages 1, 3-5 of `dev/simstudy/PLAN.md`).** Commits `6d9526d` (tier 1), `6123036` (`helper-simstudy.R`), `f63eeeb` (tiers 2-3 + runner), `2e4a5be` (deprecation). Tier 1 is 89 structural assertions in \~7 s and **ships to CRAN**; tier 2 is a `skip_on_cran()` recovery canary (\~30 s) with thresholds set from measurement across three seed sets; tier 3 is env-gated on `OCCJSDM_SIMSTUDY`, with `dev/simstudy/run_study.R` as the parallel runner (PSOCK, not `mclapply()`). Only the full R = 100 run remains.
  ...
  **Writing the tests found two package bugs that reading the code had not**, which is the argument for having done it: `sigma_s` is never sampled (now **the `sample_ls()` item in group B**) -- it exists in the simulator but only as a hard-coded `sigma_s = 1` at the `sample_ls()` call site, so the GP length-scale absorbs the amplitude misspecification and rails at the top of `l_s_grid` for every true value tried; and `predictNewSites()` still lacks its documented `X_psi`/`X_s` defaults (since fixed, **Fixed bugs 34**), correcting Fixed bugs 18. Measuring the GP knot floor also produced an item, since fixed (Fixed bugs 29).
...
4.  **Shipped data is 124 MB -- still OPEN, not yet fixed.** (Originally recorded as "`data/sampleresults.rda` is 62 MB"; see the July 25 2026 UPDATE bullet below for why the real figure is double that.) Profiled live this session: `object.size(sampleresults)` = 65.1 MB total, of which `results_output` = 64.7 MB and within that `jsdm_output` = 57 MB. The two largest single arrays are `Bs_output` (22.9 MB, dims `[30, 10, 5000, 2]`) and `U_output` (15.3 MB, dims `[100, 2, 5000, 2]`) -- i.e. full raw MCMC draws, 10,000 total (`niter=5000 x nchain=2`, `nthin=1`), stored as double-precision 4-D arrays with no thinning applied before saving. Discussed 5 options with Doug: (a) post-hoc thin the existing arrays along the `niter` axis (e.g. keep 1-in-25 draws, no refit needed, \~62MB -\> \~2.5MB) -- **recommended**, fast, doesn't change the vignette's fitted numbers structurally; (b) refit with much smaller `MCMCparams` (e.g. `niter=200`) -- same size win but requires re-running MCMC (minutes) and re-rendering vignette prose; (c) audit and drop `jsdm_output` sub-components unused by vignette/output functions (`Gs_output`/`Cs_output`/`U_output`/`As_output`/`tau_output`/`C_output`/`sigmab_output`/`sigmabs_output`/`sigmah_output`/`idx_ls_output`, \~25-30MB combined) -- combine with (a); (d) don't ship a precomputed fit at all, fit a tiny model live at vignette build time instead -- cleanest long-term but touches vignette code and the dataset's now-documented `\value`; (e) compression alone (already `LazyDataCompression: xz`) won't meaningfully shrink near-random MCMC draws. **Recommended combo**: (a) + (c). **Not yet implemented** -- next step is to check which `jsdm_output` components the vignette/output functions actually touch, then write a thin+prune script and regenerate `sampleresults.rda`.
    ...
    - **Blocked on a bug** (only if option (a) is chosen after all): the tool that plan depends on, `thinOutput()`, cannot run. It reads `niter` from `results_output$beta_ord_output`, an array that no longer exists (`R/output.R:21`), so `seq(1, NULL, by = 5)` errors immediately; it also ignores its own `thin` argument and would thin 2-D posterior-mean matrices *by row*, silently dropping sites. See the `thinOutput()` item in TODO.md group C. Fix `thinOutput()` before executing option (a).
...
7.  **OpenMP is enabled but uncapped, violating CRAN's two-core limit -- NEW (July 25 2026 audit), blocking.** (Numbered 16 to continue the single 1-15 sequence used across both subsections; it belongs here, not in "should fix".) `src/Makevars` sets `PKG_CXXFLAGS = $(SHLIB_OPENMP_CXXFLAGS)`, and there are two `#pragma omp parallel for` sites -- `samplePGvariables()` (`src/jsdm.cpp:398`) and `sample_betatheta_cpp_parallel()` (`src/functions.cpp:600`). **Nothing anywhere caps the thread count**: the only `omp_set_num_threads` in the tree is commented out (`src/jsdm.cpp:647`), so an OpenMP parallel region defaults to every available core. CRAN policy forbids using more than two cores during checks. `SystemRequirements:` also does not declare OpenMP. Fix: honour a thread limit (respect `OMP_NUM_THREADS`/`omp_set_num_threads()` bounded by 2 in checks, or expose a `nthreads` argument), and add `SystemRequirements: GNU make, OpenMP` if OpenMP is kept.
    - **Compounded by the OpenMP RNG race** (TODO.md *Fixed bugs* 26 and 28; it was a group B item renumbered twice on July 27, and before that the audit's A.6). *Partially resolved by `b7b6aa2`*: the `sample_betatheta_cpp_parallel()` region is now thread-safe (`sample_beta_nocov_cpp_TS()` calls `sample_beta_cpp_TS()`, which uses `mvrnormArmaQuick_TS()` instead of `arma::randn`). **The `samplePGvariables()` region is still affected**, via `randinvg()`'s `R::rnorm`. R's RNG state is global and not thread-safe, and this is exactly the class of defect CRAN's extra checks surface as intermittent, platform-specific failures -- active on every platform *except* stock macOS clang, i.e. everywhere CRAN checks and nowhere Doug tests. See the "OpenMP on macOS" section below for why it has never fired locally.
    - Either finish the job (route `randinvg()` through the existing `thread_local` engine at `src/jsdm.cpp:9-22` + cap threads) or drop OpenMP from `src/Makevars` entirely and parallelise over chains at the R level instead (TODO.md, *MEE paper / Alex to dos*, "Performance of `runOccJSDM()`", option A), which sidesteps this item and A.2 together.
...
16. **Package changes the user's global state -- NEW (July 25 2026 audit).** Two places, both against CRAN's "packages should not change the global state of the user's R session" policy: (a) `R/zzz.R`'s `.onLoad()` sets `options(mc.cores = min(2L, parallel::detectCores()))`, which affects every other package that reads `mc.cores` (and `detectCores()` can return `NA`, which `min(2L, NA)` propagates) -- replace with a `cores` argument on `runOccJSDM()`, see the dead-function and `globalVariables()` items in TODO.md group D; (b) `computeSpeciesDetected()` called `set.seed(1)` (`R/output.R:2222`), resetting the user's RNG stream as a side effect of drawing a plot -- **this half is now fixed**: `b7b6aa2` deleted the line (TODO.md Fixed bugs 19). Only (a) remains open.
...
- **Item 4 (shipped data size)** -- MUST BE REGENERATED, AND THE GATE MOVED (28 July): the refit now waits on the **`reparamFactorModel()`, `beta_theta` slope and `B0` bias items in group B**, all confirmed by the coverage study and all of which change fitted values. (This sentence previously glossed those three as the collection-covariate prior, the residual-correlation reparameterisation and the Stage 2 detection prior, which was a still older numbering; the collection-covariate prior mean is fixed, *Fixed bugs* 25, and the Stage 2 prior turned out not to be a defect.) Refitting before those land would bake three known-wrong quantities into the shipped example data. Previously this was gated only on the `b7b6aa2` fixes. `sampleresults.rda` was fit against buggy code, so its WAIC values, Stage 2 rates (`q`), trait coefficients (`C`/`G`/`A`), spatial parameters, and `tau` block are all stale/wrong. The now-recovered WAIC iteration-counter fix (Fixed bugs 9) is a further reason the stored WAIC is wrong. Refit with small `MCMCparams` (option b) to address both the size blocker and the correctness problem in one step, then re-render vignette. Note that post-hoc thinning via `thinOutput()` is **not** currently a safe fallback: it still thins 2-D posterior-mean matrices by row, silently dropping sites (*Fixed bugs* 16, which is only partially fixed). Fix that first if thinning is wanted.
...
2.  **Refit `sampleresults.rda` with small `MCMCparams`** (item 4, option b). **Moved behind the `reparamFactorModel()`, `beta_theta` slope and `B0` bias items in group B as of 28 July** -- the coverage study confirmed all three change fitted values, so a refit before them would ship known-wrong numbers and have to be redone. This was previously first in the phase. When it does run: the group A fixes in `b7b6aa2` change fitted values (Stage 2 rates `q`, trait coefficients `C`/`G`, spatial parameters, `tau`, and data sort order). The existing `sampleresults` is stale/wrong. Fit with `nchain=2, nburn=500, niter=1000, nthin=1` (or smaller) to address the 62 MB size blocker *and* the correctness problem in one step. ~~**Do the refit on macOS, or on a build with OpenMP disabled**, until the OpenMP RNG race is closed~~ **No longer needed:** the race is closed (*Fixed bugs* 26) and `set.seed()` now actually reaches the samplers (*Fixed bugs* 28), so the refit is reproducible on any platform. Kept because the reasoning explains why the constraint existed -- on Linux/Windows the `samplePGvariables()` RNG race makes the fit non-reproducible even at a fixed seed, which is not a property you want baked into a shipped dataset.
