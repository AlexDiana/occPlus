...
**NONE REMAINING FROM THE GROUP A (INFERENCE-AFFECTING) BUGS.** All 8 were fixed by Alex on July 26, 2026 in commit `b7b6aa2`. The `simulateOccJSDMData(model = "occupancy")` bug was pre-fixed in Alex's `0abb104`. See "Current work status" above for the commit and line numbers.
...
- **This session (July 26 2026): Alex fixed 8 inference-affecting bugs from the static audit (commit `b7b6aa2`)**. Each is a one-off edit with specific line numbers documented in TODO.Rmd's "Fixed bugs" section; no broad refactoring involved. Summary: (1) `data$OTU` not re-sorted with `data_info` (`:476-558`); (2) Stage 2 false-positive counts inflated by a catch-all `else` in `sample_pq_cpp()` (C++ version now gates cases on primer index, `:652-668`); (3) `sampleBuniv()` operator precedence drops residual precision (`:579`); (4) `Bs` read from wrong key in `list_params` (`:940`); (5) `sigma_bs` sampled but never used in prior covariance (`:711-713`); (6) `useBiotic` ignored in `computeNewOutputs()` (src C++, `:501`); (7) spatial field added after response draws instead of before (jsdmfun `:429-461`); (8) `tau_output` returned all NA (runOccJSDM output loop `:934-1236`). **All 8 are marked `FIXED` in TODO.Rmd as of this commit.** These are the same bugs from the "group A" (inference-affecting) section of the static audit from July 25 -- no new bugs discovered, just these getting addresses via direct code edits. **Implication for CRAN: `sampleresults.rda` must now be regenerated**, since bugs 2, 4, 5, 7 change fitted values (Stage 2 `q`, trait coefficients `C`/`G`, spatial parameters, `tau`); bug 1 changes the order of inference results if data is re-sorted; and bug 8 means previous fits were missing a parameter block. Doug's vignette values (WAIC, occupancy probs, etc.) will change when re-rendered against the corrected code. This is the required refit condition identified in the CRAN plan's consistency pass (July 25).
...
**Revised July 26 2026** after Alex's fix of 8 group A inference bugs (commit `b7b6aa2`, see "Current work status" above). Nothing below was re-derived from a fresh `devtools::check()` run -- item 13 is still outstanding.
...
- **Item 4 (shipped data size)** -- MUST BE REGENERATED FIRST: `sampleresults.rda` was fit against buggy code (8 group A bugs fixed in `b7b6aa2`), so its WAIC values, Stage 2 rates (`q`), trait coefficients (`C`/`G`/`A`), spatial parameters, and `tau` block are all stale/wrong. Refit with small `MCMCparams` (option b) to address both the size blocker and the correctness problem in one step, then re-render vignette. Only after this is done should any post-hoc thinning (`thinOutput()`) be considered.
- **Item 16 (OpenMP uncapped)** -- violates CRAN's two-core policy and is entangled with the RNG thread-safety bug (group B item 6 in TODO.Rmd). Either cap threads + fix RNG thread-safety, or drop OpenMP and parallelise over chains in R instead (latter is lower-risk per TODO.Rmd D.1).

Of the should-fix items: 7 and 15 are fixed; 11 is closed by deliberate decision (keep `LICENSE`, accept the cosmetic NOTE); 9 is now **closed as fixed** (group C cleanup in TODO.Rmd removes 4 of the 7 compiler warnings; group A fix 2 removes 3 of 7 for `sample_pq_cpp()`'s catch-all); 8, 10, 12, 13, 14 are open; 17-20 were added by the July 25 audit; and 21-23 were added from the occPlus review. Item 21 (`parallel` undeclared) is the most severe of the new batch -- it is a check WARNING. Item 19 **corrects** the earlier characterisation of the `tidyr` NOTE (it is a *missing-import*, not an *unused-import*). The `stats`-import portion of the "no visible binding" NOTE is fixed (`ef77c4c`); the NSE-column portion is deliberately left unfixed by the user's decision; item 20 catalogues a further source of that NOTE (dead functions referencing undefined globals).

### Recommended order (bug fixes done, refit priority now)

**UPDATED: July 26 2026 -- group A bug fixes are now committed (Alex, `b7b6aa2`).** The order below is revised accordingly. The critical blocking item is now **item 4 (regenerate `sampleresults.rda`)**, which is stale from having been fit against buggy code.
...
**Phase 1 -- Regenerate artefacts invalidated by the July 26 bug fixes.**

2. **Refit `sampleresults.rda` with small `MCMCparams`** (item 4, option b). This must come first: the 8 group A fixes in `b7b6aa2` change fitted values (Stage 2 rates `q`, trait coefficients `C`/`G`, spatial parameters, `tau`, and data sort order). The existing `sampleresults` is stale/wrong. Fit with `nchain=2, nburn=500, niter=1000, nthin=1` (or smaller) to address the 62 MB size blocker *and* the correctness problem in one step.
3. **Re-render `vignettes/occJSDM.Rmd`** -- its model-selection prose hard-codes `extractWAIC()` values that changed with the A.4 fix, and all posterior-based numbers will shift.
...
**Phase 2 -- Remaining bug fixes (groups B and C from TODO.Rmd).**

5. **TODO.Rmd group B (crashes and API bugs).** Item 8 (`\dontrun{}` -> `\donttest{}`) depends on this (see the ordering constraint on that item).
6. **TODO.Rmd group C (dead code).** This single cleanup absorbs **item 20** (no-visible-binding NOTEs), **item 9's** compiler warnings from dead `isPointInBand*` helpers, **item 22's** duplicate `buildGrid()`, and **item 19** in full (the `tidyr`/`splines`/`dnbinom` callers are dead -- deleting beats importing). After this, item 9 needs no dedicated work at all.
7. **Decide the OpenMP question (item 16)**: cap threads and make the RNG thread-safe (TODO.Rmd A.6), *or* drop OpenMP from `src/Makevars` and parallelise over chains in R instead (TODO.Rmd D.1). The second closes item 16 and A.6 together and is the lower-risk path.
8. **Drop `options(mc.cores = ...)` from `.onLoad()`.** One edit closing three items: **17(a)**, **21** (it is the only `parallel::` call), and TODO.Rmd **D.2**.
...
11. **Write the testthat suite.** Sequence it here so the tests encode fixed behaviour rather than pinning the old bugs; the simulation-recovery work under *MEE paper / Doug to dos* in TODO.Rmd is the natural vehicle.
